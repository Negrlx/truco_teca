...
namespace truco_teca
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("3. Simulacro manual (definir Vira y cartas)");
            Console.Write("Opción: ");

            string? modeInput = Console.ReadLine();

            bool simulationMode = modeInput == "3";

            int playersCount = modeInput == "2" ? 4 : 2;
            if (simulationMode) playersCount = 2; // Simulacro simple 1v1 (puedes ampliar si deseas)
            ...
            int pointsToWin = 1; // Esto no importa en simulación, pero lo dejamos
            if (!simulationMode)
            {
            ...
            {
                Console.WriteLine("\n=== Modo Simulacro Activado ===\n");
            }

            Game game = new Game(players, pointsToWin);

            if (simulationMode)
                game.RunSimulation(); // Método que vas agregar a Game
            else
                game.Start();
